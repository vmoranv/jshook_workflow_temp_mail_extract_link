...
export default createWorkflow(workflowId, 'Temp Mail Extract Link')
    ...
    const titleBlocklist = ctx.getConfig(`${prefix}.titleBlocklist`, [
        'Redirecting',
    ]);
    const bodyBlocklist = ctx.getConfig(`${prefix}.bodyBlocklist`, [
    ...
    const includeFallbackLinks = Boolean(ctx.getConfig(`${prefix}.includeFallbackLinks`, true));
    ...
    root
        .branch('maybe-navigate-detail', 'temp_mail_extract_link_has_detail_url', (b) => {
        b.predicateFn(() => Boolean(detailUrl))
            .whenTrue(new ToolNodeBuilder('navigate-detail', 'page_navigate').input({
            url: detailUrl,
            waitUntil,
            enableNetworkMonitoring: true,
        }))
            .whenFalse(new ToolNodeBuilder('skip-navigate-detail', 'console_execute').input({
        ...
        .branch('maybe-wait-after-open', 'temp_mail_extract_link_wait_after_open', (b) => {
        b.predicateFn(() => openFirstMatch)
            .whenTrue(new ToolNodeBuilder('wait-after-open', 'page_evaluate')
            .input({
            ...
            .whenFalse(new ToolNodeBuilder('skip-wait-after-open', 'console_execute').input({
